...
const DATA_POLICY_BANNER = 'https://res.cloudinary.com/dqqeavica/image/upload/v1766347982/banner_wbfbf7.gif';
  ...
  async function validarContratoParaIngresarCuenta_(){
  if(!currentUser || !currentUser.documento){
    return { ok:false, reason:'sesion' };
  }
  const res = await apiGet('canCrearCuenta', {
...
if (window.Swal && typeof Swal.fire === 'function'){
 ...
 //Reproduce un audio "extra" 1 sola vez, DESPUÉS del sonido normal del icon (info/success/etc)
 // y SOLO cuando el SweetAlert se abre (didOpen)
  
function swalExtraSoundOnceAfterIcon_(options, extraUrl){
  const prevDidOpen = options.didOpen;

  options.didOpen = function(popup){
    // 1) deja que Swal haga lo suyo primero
    try{ if(typeof prevDidOpen === 'function') prevDidOpen(popup); }catch(_){}

    // 2) reproduce el extra 1 sola vez por alerta
    try{
      if(popup && popup.dataset && popup.dataset.extraSoundPlayed === '1') return;
      if(popup && popup.dataset) popup.dataset.extraSoundPlayed = '1';

      // espera un pelo para que el sonido del icono (info/success) salga primero
      setTimeout(()=>{ playSoundOnce(extraUrl); }, 220);
    }catch(_){}
  };
...
/* ================== GUÍA FLOTANTE + MODAL (INGRESAR / CORREGIR CUENTA) ================== */
(function initGuiaCuenta_(){
  const FLOAT_ID = 'guiaFloat';
  const MODAL_ID = 'guiaModal';

  function el_(id){ return document.getElementById(id); }

  // 1) Mapa: TÍTULO DEL BLOQUE -> AUDIO
  // IMPORTANTE: las llaves deben coincidir con data.title en MAYÚSCULAS
  const GUIA_AUDIO_BY_TITLE = {
    'RELACIÓN DE FECHAS': 'https://res.cloudinary.com/dqqeavica/video/upload/v1772593271/fechas_nellk8.mp3',
    'RELACIÓN DE PAGO': 'https://res.cloudinary.com/dqqeavica/video/upload/v1772593525/pagos_daw8tc.mp3',
    'RELACIÓN DE PLANILLA': 'https://res.cloudinary.com/dqqeavica/video/upload/v1772593710/planilla_jdl3al.mp3',
    'RELACIÓN DE PAGO PLANILLA ANEXA': 'https://res.cloudinary.com/dqqeavica/video/upload/v1772594118/planilla_anexa_yjqcxw.mp3',
    'RELACIÓN DE ACTIVIDADES': 'https://res.cloudinary.com/dqqeavica/video/upload/v1772596388/actividades_egx9hk.mp3',
    'CARGUE DE ARCHIVOS DE CUENTA': 'https://res.cloudinary.com/dqqeavica/video/upload/v1772594394/archivos_cuenta_kzy6y3.mp3',
    'LOS SIGUIENTES DOCUMENTOS SON OPCIONALES': 'https://res.cloudinary.com/dqqeavica/video/upload/v1772594866/doc_opcionales_usv2ay.mp3',
    'DOCUMENTOS PARA PERSONAS REGIMEN SIMPLE, JURÍDICAS, NO APORTES A PENSIÓN':'https://res.cloudinary.com/dqqeavica/video/upload/v1772594659/doc_regimen_dgnqzm.mp3',
    'DOCUMENTOS ANEXOS (PRIMERA Y ÚLTIMA CUENTA, NOVEDADES)': 'https://res.cloudinary.com/dqqeavica/video/upload/v1772595248/primera_tesfpf.mp3',
    'GUÍA': 'https://res.cloudinary.com/dqqeavica/video/upload/v1772627036/friend_e1qluo.mp3'
  };

  // 2) Un solo reproductor reutilizable (evita solapes)
  let __guiaAudio = null;

  function stopGuiaAudio_(){
    try{
      if(__guiaAudio){
        __guiaAudio.pause();
        __guiaAudio.currentTime = 0; // corta el audio
      }
    }catch(_){}
  }

  async function playGuiaAudioForTitle_(title){
    try{
      const key = String(title || '').trim().toUpperCase();
      const url = GUIA_AUDIO_BY_TITLE[key];
      if(!url){
        stopGuiaAudio_();
        return;
      }
      ...
      if(!__guiaAudio){
        __guiaAudio = new Audio();
        __guiaAudio.preload = 'auto';
      }

      __guiaAudio.src = url;

      // Esto “autoplay” al abrir modal (pero realmente está autorizado por el toque del usuario)
      await __guiaAudio.play();
    }catch(_){
      // Si algún navegador lo bloquea, no rompemos la app.
      // (si quieres, aquí podrías mostrar un aviso “toca para activar audio”)
    }
  }

  function isGuiaViewActive_(){
    return !!document.getElementById('view-ingresar-cuenta')?.classList.contains('active');
  }

  function showGuiaFloat_(){
    const f = el_(FLOAT_ID);
    if(!f) return;
    f.style.display = isGuiaViewActive_() ? 'block' : 'none';
    f.setAttribute('aria-hidden', isGuiaViewActive_() ? 'false' : 'true');
  }
  
  function guiaContentBySection_(){
    // Detecta en qué parte del formulario está el usuario, mirando el heading más cercano al centro de pantalla
    const view = document.getElementById('view-ingresar-cuenta');
    if(!view) return { title:'GUÍA', html:'' };

    const headings = Array.from(view.querySelectorAll('h3'));
    if(!headings.length) return { title:'GUÍA', html:'' };
    ...
    const t = (best.textContent || '').trim().toUpperCase();

    const blocks = {
      'RELACIÓN DE FECHAS': `
        <h4>RELACIÓN DE FECHAS</h4>
        <div class="guia-box">
        <p><b>Si es tu primera cuenta, corrobora que Total de Informes esté correcto</b></p>
          <p>Ingresa de forma correcta:</p>
          <ul>
            <li><b>Inicio de Periodo Relacionado</b> Ejemplo: 20/01/2026</li>
            <li><b>Fin de Periodo Relacionado</b> Ejemplo: 19/02/2026</li>
            <li>Fecha de Radicación</li>
          </ul>
          <p><b>Recuerda:</b> la <b>Fecha de Radicación</b> debe estar <b>fuera</b> del periodo relacionado.</p>
        </div>
      `,
      'RELACIÓN DE PAGO': `
        <h4>RELACIÓN DE PAGO</h4>
        <div class="guia-box">
          <p><b>Si es la primera cuenta</b> digita el valor total de tu contrato en <b>Saldo Actual</b>.</p>
          <p>Ingresa el valor a cobrar.</p>
          <p>El campo nuevo saldo es automático.</p>
          <p><b>Factura Electrónica:</b> si no manejas, deja el campo vacío.</p>
        </div>
      `,
      'RELACIÓN DE PLANILLA': `
        <h4>RELACIÓN DE PLANILLA</h4>
        <div class="guia-box">
         <p><b>Todos los datos deben coincidar con la planilla YA pagada.</b></p>
          <ul>
            <li>Revisa en la planilla el número de la misma.</li>
            <li>Base de cotización mínima para mes completo: <b>$ 1.750.905</b>. Sin embargo, corrobora en tu planilla, todos los valores ingresados deben coincidir.</li>
            <li>Ingresa la base de cotización que aparece en tu planilla..</li>
            <li>Ingresa el aporte a salud que aparece en tu planilla.</li>
            <li>Ingresa el aporte a pensión que aparece en tu planilla. Si no haces aportes a pensión, escribe <b>0</b> NO dejes el campo vacio.</li>
            <li>Ingresa el aporte a riesgos que aparece en tu planilla. <b>si la alcalía paga tus riesgos</b>, escribe el valor del aporte <b>(no dejar en 0 ó vacío)</b>.</li>
            <li>Si no eres persona jurídica, NO diligencies el resto de campos como caja de compensación, SENA e ICBF.</li>
          </ul>
        </div>
      `,
      'RELACIÓN DE PAGO PLANILLA ANEXA': `
        <h4>RELACIÓN DE PAGO PLANILLA ANEXA</h4>
        <div class="guia-box">
        <p><b>Ingresa estos valores si vas a relacionar una segunda planilla en esta cuenta de cobro.</b></p>
        <p>Todos los datos deben coincidir con la planilla Anexa YA pagada.</p>
          <ul>
            <li>Revisa en la planilla el número de la misma.</li>
            <li>Base de cotización mínima para mes completo: <b>$ 1.750.905</b>. Sin embargo, corrobora en tu planilla, todos los valores ingresados deben coincidir.</li>
            <li>Ingresa la base de cotización que aparece en tu planilla..</li>
      ...
      'RELACIÓN DE ACTIVIDADES': `
        <h4>RELACIÓN DE ACTIVIDADES</h4>
        <div class="guia-box">
          <p><b>Ten en cuenta estos puntos para que tu informe de actividades cumpla los estándares de aprobación:</b></p>
          <ol>
            <li>Relaciona metas del Plan de Desarrollo (si aplica).</li>
            <li>Lugares de realización (barrios, veredas, conjuntos, oficina, etc.).</li>
            <li>Relación de cantidades (Atención al Público, Revisiones, limpiezas, estudios, creación de archivos, recepción y envío de Documentos, visitas, reuniones, suministros entregados, trabajos realizados, etc.).</li>
            <li>N° personas impactadas.</li>
            <li>Medios de corroboración (Link página de Facebook de la Alcaldía, ANEXOS, descripción de archivos, etc.).</li>
            <li><b>Solo se permite una imagen por Evidencia, si tienes 2 o 3 como máximo; debes crear un collage sin rebordes ni espacios y subirlo como una sola imagen.</b>.</li>
          </ol>
          <pSi tienes archivos por anexar, debes unificarlos en un solo PDF y subirlos en el campo <b>Anexos de Actividades</b>.</p>
          <p><b>Pre-avisos:</b></p>
          <ol>
            <li>No copies las mismas actividades en cada cuenta (están bajo seguimiento).</li>
            <li>Las cuentas que no cumplan los parámetros según los casos, serán devueltas.</li>
            <li>Los supervisores no pueden aprobar cuentas que no cumplan con los criterios anteriores.</li>
          </ol>
        </div>
      `,
      'CARGUE DE ARCHIVOS DE CUENTA': `
        <h4>CARGUE DE ARCHIVOS DE CUENTA</h4>
        <div class="guia-box">
        <p>Debes cargar los siguientes archivos en todas tus cuentas.</p>
          <ul>
            <li><b>Anexos de Actividades:</b> para archivos diferentes a imágenes en los casos que aplique o sugiera tu Supervisor.</li>
            <li><b>Certificación Bancaria:</b> debes quitar la contraseña de este archivo antes de adjuntarlo.</li>
            <li><b>Baucher Planilla:</b> adjunta el recibo ó comprobante de pago.</li>
            <li><b>Planilla:</b> adjunta la planilla ya pagada y sin contraseña.</li>
          </ul>
          <p><b>Tienes botones para unificar y desbloquear archivos.</p>
        </div>
      `,
      'LOS SIGUIENTES DOCUMENTOS SON OPCIONALES': `
        <h4>DOCUMENTOS OPCIONALES</h4>
        <div class="guia-box">
         <p>Si relacionaste otra planilla de pago, debes adjuntar también:</p>
          <ul>
            <li><b>Baucher Planilla Anexa:</b> comprobante de pago.</li>
            <li><b>Planilla Anexa:</b> adjunta ya pagada y sin contraseña.</li>
          </ul>
        </div>
      `,
      'DOCUMENTOS PARA PERSONAS REGIMEN SIMPLE, JURÍDICAS, NO APORTES A PENSIÓN': `
        <h4>DOCUMENTOS (RÉGIMEN SIMPLE / JURÍDICAS / NO APORTES A PENSIÓN)</h4>
        <div class="guia-box">
        <p>Deben anexar los siguientes documentos en los casos que aplique:</p>
          <ul>
            <li><b>RUT (Régimen Simple):</b> Si eres Persona Natural, no adjuntes el RUT en esta parte.</li>
            <li><b>Factura Electrónica:</b> revisa muy bien antes de adjuntar.</li>
            <li><b>Certificado Parafiscales:</b> aplica para personas jurídicas.</li>
            <li><b>Certificado NO aportes a Pensión:</b> para quienes no realizan ese aporte.</li>
          </ul>
           <p>Estos archivos según el caso,se adjuntas en todas las cuentas, si no aplicas, no adjuntes ningún archivo de forma equivoca</p>
        </div>
      `,
      'DOCUMENTOS ANEXOS (PRIMERA Y/O ÚLTIMA CUENTA, NOVEDADES)': `
        <h4>DOCUMENTOS ANEXOS (PRIMERA / ÚLTIMA / NOVEDADES)</h4>
        <div class="guia-box">
          <p>Adjunta estos archivos según el caso.:</p>
          <p><b>Primera cuenta:</b> Acta de Inicio, Clausulados, CDP, RP, RUT PN, Certificado ARL (no para personas jurídicas).</p>
          <p><b>Casos puntuales:</b> OTROSI (Modificaciones al contrato), Otro Requerido (según solicitud).</p>
          <p><b>Primera cuenta de adición:</b> CDP Adición, RP Adición.</p>
          <p><b>Última cuenta:</b> Acta de Liquidación.</p>
        </div>
      `
    };
  ...
  const html = blocks[t] || `
      <h4>GUÍA</h4>
      <div class="guia-box">
        <p>Ubícate al lado de uno de los títulos y vuelve a tocarme, <b>soy Digital Friend</b>.</p>
      </div>
    `;

return { title: t || 'GUÍA', label: 'GUÍA', html };
}

 function openGuiaModal_(){
    const modal = el_(MODAL_ID);
    const titleEl = el_('guiaModalTitle');
    const body  = el_('guiaModalBody');
    if(!modal || !titleEl || !body) return;

    const data = guiaContentBySection_();
    const title = data.title || 'GUÍA';

    titleEl.textContent = title;
    body.innerHTML = data.html || '';
    modal.setAttribute('aria-hidden','false');

    // 3) Arranca audio según el bloque abierto
  playGuiaAudioForTitle_(data.title);
}

  function closeGuiaModal_(){
    const modal = el_(MODAL_ID);
    if(!modal) return;
    modal.setAttribute('aria-hidden','true');

    // 4) Al cerrar: cortar audio
    stopGuiaAudio_();
  }

  // Eventos
  el_('guiaFloatImg')?.addEventListener('click', ()=>{
    try{ playSoundOnce(SOUNDS.back); }catch(_){}
    openGuiaModal_();
  });

  el_('guiaModalClose')?.addEventListener('click', ()=>{
    try{ playSoundOnce(SOUNDS.back); }catch(_){}
    closeGuiaModal_();
  });


  el_(MODAL_ID)?.addEventListener('click', (e)=>{
...
// Si cambias de vista: cerrar modal y cortar audio
 ...
 // primer estado por si ya está activa
  document.addEventListener('DOMContentLoaded', ()=>{
    showGuiaFloat_();
  });

  // por si se hace scroll dentro de la vista (refresca visibilidad)
  window.addEventListener('scroll', ()=>{
    if(isGuiaViewActive_()){
      showGuiaFloat_();
    }
  }, { passive:true });
    ...
    /* ================== Guía Flotante (inicio) ================== */

  (function initGuiaInicio_(){
  const FLOAT_ID = 'guiaInicioFloat';
  const MODAL_ID = 'guiaInicioModal';
  const AUDIO_URL = 'https://res.cloudinary.com/dqqeavica/video/upload/v1772627222/bienvenida_sdnyae.mp3';

  let __audio = null;

  function el_(id){ return document.getElementById(id); }
  function isInicioActive_(){
    return !!document.getElementById('view-inicio')?.classList.contains('active');
  }
  ...
  // Click en el ícono
  el_('guiaInicioFloatImg')?.addEventListener('click', ()=>{
    try{ playSoundOnce(SOUNDS.back); }catch(_){}
    openModal_();
  });

  // Cerrar
  el_('guiaInicioModalClose')?.addEventListener('click', ()=>{
    try{ playSoundOnce(SOUNDS.back); }catch(_){}
    closeModal_();
  });
  ...
  // Hook showView para refrescar visibilidad y cortar audio al cambiar de vista
  if(typeof window.showView === 'function'){
    const __orig = window.showView;
    window.showView = function(id){
      __orig(id);
  ...
  document.addEventListener('DOMContentLoaded', ()=>{ showFloat_(); });
})();

  /* ================== IMÁGENES (preview y compresión) ================== */
...
async function isEncryptedPdfFile_(file){
  ...
  try{
    ...
    // leer más del encabezado para aumentar detección (1MB máx => seguro)
    ...
    // Algunos PDFs protegidos también incluyen /Filter /Standard y /SubFilter
...
function bindPdfInputStrict(id, { required=false, forbidEncrypted=false, maxMb=MAX_PDF_MB } = {}){
  ...
  function setNewFileUi_(file){
    ...
    textSpan.textContent = name + ': archivo cargado';
    ...
    if(forbidEncrypted){
      ...
      if(encrypted){
        ...
        await Swal.fire({
          ...
          text:'Este archivo parece estar protegido/cifrado con contraseña. Por favor exporta o guarda una versión sin contraseña e inténtalo nuevamente.'
        });
        return;
      }
    }
...
async function collectBankPlanPdfsBase64_(){
 const ids = ['bancaria','baucher1','planaporte1','baucher2','planaporte2','rutsg','factElect','otroPdf','noafp','actAnexos'];
  const out = {};
  ...
  for(const id of ids){
    ...
    // En corrección: si el usuario presionó "Quitar" sobre un precargado, mandamos marca de borrado
...
/* ================== LOGIN OVERLAY LOADER (CONECTANDO...) ================== */
/** Loader overlay exclusivo para el flujo de login (no se reinicia por llamadas API internas). */
let __loginOverlayOpen = false;

function showConnectingLoginLoader_(){
  if(__loginOverlayOpen) return;
  __loginOverlayOpen = true;

  try{
    suppressLoader = true; // evita que el loader global parpadee durante el login
    loadingCount = 0;
    if(loaderTimer){ clearTimeout(loaderTimer); loaderTimer = null; }
    if(loader){
      loader.classList.add('hidden');
...
function hideConnectingLoginLoader_(){
  ...
  // Reactiva loader global para el resto de la app
  try{
...
/* Capa de red endurecida.
   Motivo: /exec de Apps Script responde con un 302 hacia
   script.googleusercontent.com con una clave de un solo uso. Si esa
   respuesta se pierde o se reenvía, Google devuelve una PÁGINA HTML
   (404/500) y el `r.json()` de antes reventaba con el mensaje críptico
   "Unexpected token '<', "<!DOCTYPE "... is not valid JSON".
   Aquí leemos primero como texto, detectamos HTML, avisamos en claro y
   reintentamos SOLO lo que es seguro reintentar (GET). */

/* Un GET que tarda 45 s y se reintenta 3 veces deja al contratista mirando el
   reloj más de 2 minutos antes de ver siquiera el error. Con el backend ya
   recortado, ninguna consulta pasa de unos segundos: 30 s es de sobra y el
   peor caso baja de ~137 s a ~92 s. */
const API_TIMEOUT_MS        = 30000;   // GET normales
const API_TIMEOUT_POST_MS   = 120000;  // POST normales (2 min)
const API_TIMEOUT_PESADO_MS = 420000;  // 7 min: sube archivos a Drive y regenera documentos
const API_MAX_INTENTOS      = 3;       // solo para GET (idempotente)

/* Acciones que suben PDFs/imágenes a Drive y/o regeneran documentos.
   La propia app le avisa al usuario "entre 1.50 y 2.30 minutos", así que
   NO pueden compartir el timeout corto de una consulta. */
const API_ACCIONES_PESADAS = [
  'savenuevacuenta', 'savecorreccioncuenta', 'generardocumentoscuenta',
  'saveregistro', 'savedatoscontrato', 'savedatospersonales',
  'guardarsoportecontratista', 'guardarsolicitudtesoreria',
  'guardarsolicitudprensabrief', 'generarcertificacioncontrato'
];

function apiTimeoutDe_(action, metodo, tamBody){
  const a = String(action || '').toLowerCase();
  if(API_ACCIONES_PESADAS.indexOf(a) !== -1) return API_TIMEOUT_PESADO_MS;
  // Si el cuerpo lleva archivos en base64, también es pesada aunque no esté en la lista.
  if(Number(tamBody || 0) > 200000) return API_TIMEOUT_PESADO_MS;
  return (metodo === 'POST') ? API_TIMEOUT_POST_MS : API_TIMEOUT_MS;
}

function apiEsHtml_(txt){
  const t = String(txt || '').replace(/^\uFEFF/, '').trimStart().slice(0, 300).toLowerCase();
  return t.startsWith('<!doctype') || t.startsWith('<html') || t.startsWith('<');
}

function apiErrorReintentable_(msg){
  const e = new Error(msg);
  e.__apiReintentable = true;
  return e;
}

/**
 * Pide JSON al backend con timeout, detección de HTML y reintentos opcionales.
 * @param {string} url
 * @param {object} opts       opciones de fetch
 * @param {string} etiqueta   nombre de la acción (para mensajes y consola)
 * @param {number} maxIntentos 1 = sin reintentos (obligatorio en POST)
 * @param {number} timeoutMs   espera máxima antes de rendirse
 */
async function apiFetchJson_(url, opts = {}, etiqueta = '', maxIntentos = 1, timeoutMs = API_TIMEOUT_MS){
  const esEscritura = String(opts.method || 'GET').toUpperCase() === 'POST';
  ...
  for(let intento = 1; intento <= maxIntentos; intento++){
    ...
    try{
      const r = await fetch(url, Object.assign({
        credentials: 'omit',   // nunca mandar cookies de Google (evita redirecciones /u/N)
        redirect: 'follow',
        cache: 'no-store',
        signal: ctrl.signal
      }, opts));
      ...
      if(!r.ok){
        console.warn('[API] HTTP ' + r.status, etiqueta, url);
        ultimoError = apiErrorReintentable_(
          'El servidor respondió ' + r.status + (etiqueta ? ' en "' + etiqueta + '"' : '') + '. Intenta de nuevo.'
        );
      }else if(apiEsHtml_(txt)){
        console.warn('[API] respuesta HTML inesperada', etiqueta, url, txt.slice(0, 300));
        ultimoError = apiErrorReintentable_(
          'El servidor devolvió una página web en lugar de datos' +
          (etiqueta ? ' en "' + etiqueta + '"' : '') + '. Intenta de nuevo.'
        );
      }else{
        let j = null, roto = false;
        try{ j = JSON.parse(txt); }catch(_){ roto = true; }

        if(roto || !j || typeof j !== 'object'){
          console.warn('[API] JSON ilegible', etiqueta, url, String(txt).slice(0, 300));
          ultimoError = apiErrorReintentable_(
            'El servidor devolvió una respuesta ilegible' +
            (etiqueta ? ' en "' + etiqueta + '"' : '') + '. Intenta de nuevo.'
          );
        }else if(!j.ok){
          // Error de negocio del backend: NO se reintenta, se muestra tal cual.
          const eNeg = new Error(j.error || 'Error');
          eNeg.__apiNegocio = true;
          throw eNeg;
        }else{
          return j.data;
        }
      ...
      if(e && e.__apiNegocio) throw e;          // error real del backend
      if(e && e.__apiReintentable){ ultimoError = e; }
      else if(e && e.name === 'AbortError'){
        console.warn('[API] timeout ' + Math.round(timeoutMs/1000) + 's', etiqueta, url);
        ultimoError = apiErrorReintentable_(
          esEscritura
            // En un POST el servidor pudo haber terminado el trabajo aunque el
            // navegador se cansara de esperar. NO afirmar que falló: reenviar
            // a ciegas duplicaría el registro y los archivos en Drive.
            ? 'El servidor está tardando más de lo normal. Es posible que el guardado SÍ se haya completado: revísalo antes de volver a enviarlo, para no duplicarlo.'
            : 'El servidor tardó demasiado en responder. Revisa tu conexión e intenta de nuevo.'
        );
      }else{
        console.warn('[API] fallo de red', etiqueta, url, e);
        ultimoError = apiErrorReintentable_('No hay conexión con el servidor. Revisa tu internet e intenta de nuevo.');
      }
    }finally{
      clearTimeout(to);
    }
    ...
    if(intento < maxIntentos){
      await new Promise(res => setTimeout(res, 700 * intento)); // espera creciente
    }
  }
...
async function apiGet(action, params = {}){
  ...
  try{
    ...
    // GET es idempotente: se puede reintentar sin riesgo de duplicar nada.
    return await apiFetchJson_(url.toString(), { method: 'GET' }, action,
      API_MAX_INTENTOS, apiTimeoutDe_(action, 'GET', 0));
  } finally { stopLoading(); }
}
...
async function apiPost(action, body = {}){
  ...
  try{
    ...
    // OJO: POST escribe en la hoja. NO se reintenta nunca (duplicaría registros).
    return await apiFetchJson_(url, {
      method:'POST',
      headers: { 'Content-Type':'text/plain;charset=utf-8' },
      body: cuerpo
    }, action, 1, apiTimeoutDe_(action, 'POST', cuerpo.length));
  } finally { stopLoading(); }
}
/* ============================================================================
   RED DE SEGURIDAD DEL GUARDADO DE CUENTAS
   ----------------------------------------------------------------------------
   Apps Script contesta el POST con una redirección de un solo uso. Cuando el
   guardado tarda, esa segunda petición se pierde y el navegador recibe un 404
   AUNQUE EL SERVIDOR YA HAYA TERMINADO. Antes salía "El servidor respondió 404"
   y la app se quedaba en la misma pantalla, así que el contratista volvía a
   guardar y duplicaba los archivos en Drive.

   Ahora, cuando un guardado se cae por red (404, HTML, timeout), se le pregunta
   al servidor en qué quedó la fila y se actúa según la respuesta. Los errores
   de negocio del backend (los que llegan con __apiNegocio) NO pasan por aquí:
   esos son mensajes legítimos y se muestran tal cual.
   ========================================================================== */
async function confirmarGuardadoCuenta_(informe){
  try{
    const r = await apiGet('confirmarGuardadoCuenta', {
      documento: currentUser?.documento || '',
      supervisor: currentUser?.supervisor || '',
      secretaria: currentUser?.secretaria || '',
      informe: String(informe || '')
    });
    return (r && r.resultado) ? r.resultado : 'desconocido';
  }catch(_){
    return 'desconocido';
  }
}

function mostrarExitoCuenta_(isCorrection){
  return Swal.fire({
    icon:'success',
...
/* Los avisos al GRUPO del supervisor (reportar cuenta, corrección, plan de pagos
   y corrección de plan) YA NO salen de aquí: los manda el backend dentro de la
   misma acción que cambia el estado. Este helper solo muestra la advertencia
   cuando el backend informa que el mensaje no pudo salir, para que el fallo
   deje de ser invisible como venía pasando. */
async function avisarSiFalloElGrupo_(respuesta){
  const aviso = respuesta && respuesta.avisoGrupo;
  if(!aviso || aviso.success) return;

  console.warn('[WhatsApp] aviso al grupo no enviado', aviso);

  await Swal.fire({
    icon: 'warning',
    title: 'AVISO NO ENVIADO AL SUPERVISOR',
    html: 'Tu reporte <b>sí quedó registrado</b>, pero el mensaje de WhatsApp al grupo de tu supervisor(a) no pudo enviarse.<br><br>' +
          'Infórmalo a la Oficina de Contratación para que revisen la notificación.',
...
function resetVistas(){
  ...
  // Limpiar caches locales del feed visual (no borra leído)
...
function bindNumericSanitizer(id, maxLen){
  ...
  // Helper simple: detecta si es móvil (iOS/Android)
function isMobileDevice(){
  ...
  <div style="text-align:center;">
    <div style="font-weight:900; font-size:1rem; margin-bottom:10px;">
      ...
      App Contratista – Alcaldía de Flandes
    </div>
    ...
    ">
      ...
      La <b>Alcaldía Municipal de Flandes – Tolima</b> es responsable del tratamiento de los datos personales recolectados a través de la aplicación <b>App Contratista</b>, usada para la gestión digital de cuentas de cobro de los contratistas vinculados mediante <b>contratos estatales de prestación de servicios</b>.<br><br>

      <b>2. Marco Legal</b><br>
      Esta política se rige por:<br>
      - Artículo 15 de la Constitución Política.<br>
      - Ley 1581 de 2012 y Decreto 1377 de 2013.<br>
      - Decreto 1074 de 2015.<br>
      - Ley 1712 de 2014 (Transparencia).<br>
      - Ley 80 de 1993 y Ley 1150 de 2007 (Contratación estatal).<br>
      ...
      - Publicación posterior en la plataforma <b>SECOP II</b>, en cumplimiento del deber legal de publicidad de la contratación pública.<br><br>

      <b>4. Datos Tratados</b><br>
      Se recolectan:<br>
      - Dirección, teléfono y correo electrónico.<br>
      - Fechas de inicio y terminación del contrato.<br>
      - Valores a cobrar.<br>
      - Relación de planillas.<br>
      - Actividades ejecutadas.<br>
      - Imagen de la firma manuscrita.<br>
      No se recolectan datos sensibles.<br><br>

      <b>5. Información de Carácter Público</b><br>
      Debido a la naturaleza del <b>contrato estatal de prestación de servicios</b>, parte de la información contenida en los documentos generados deberá ser publicada en el <b>Sistema Electrónico para la Contratación Pública – SECOP II</b>, plataforma administrada por Colombia Compra Eficiente y accesible al público, conforme a la Ley 1712 de 2014 y demás normas aplicables.<br><br>

      <b>6. Derechos del Titular</b><br>
      El contratista puede ejercer los derechos de habeas data: conocer, actualizar, rectificar, revocar la autorización cuando sea procedente.<br><br>

      <b>7. Seguridad de la Información</b><br>
      La Alcaldía adopta medidas técnicas y administrativas razonables para proteger los datos durante su tratamiento en la aplicación.<br><br>

      <b>8. Conservación</b><br>
      La información será conservada conforme a las normas de archivo público y los términos contractuales y fiscales aplicables.<br><br>

      <b>9. Aceptación del Titular</b><br>
      Antes de usar la aplicación, el contratista deberá manifestar su voluntad mediante la siguiente declaración:<br><br>

      <b>AUTORIZACIÓN DE TRATAMIENTO DE DATOS PERSONALES</b><br><br>

      Declaro que he leído y comprendido la presente Política de Tratamiento de Datos Personales de la <b>Alcaldía de Flandes</b> para la aplicación <b>App Contratista</b>, desarrollada para la gestión de contratos estatales de prestación de servicios en la presente adminitración, y <b>AUTORIZO de manera previa, expresa e informada</b> el tratamiento de mis datos personales, incluida la generación de documentos y su publicación en el SECOP II cuando la ley lo exija.
    </div>
...
const HOME_BANNERS = [
    ...
    img: 'https://res.cloudinary.com/dqqeavica/image/upload/v1773231775/denegado_icj134.png',
    modal: {
      heroImg: 'https://res.cloudinary.com/dqqeavica/image/upload/v1773232132/correo_ei7azu.png', 
      html: `
        ...
        <div style="
          ...
          color:#111;
        ">
          Que estés usando el correo registrado en la aplicación, haz clic donde te señalo en la imagen y usa el correo registrado.
        </div>
  ...
  {
    id: 'no_colgarte',
    title: '¿Por qué no debes colgarte con tus cuentas de cobro?',
    img: 'https://res.cloudinary.com/dqqeavica/image/upload/v1772417404/DATOS_DEL_CONTRATO_mlmwhi.png',
    modal: {
      heroImg: '', // opcional: si luego quieres imagen aquí, coloca url
      html: `
        <p style="margin-top:0;">
          No puedes colgarte ni acumular cuentas de cobro debido a que una de las obligaciones de tu contrato es:
        </p>
        <div style="
          background:#f8fafc;
          border:2px solid #e5e7eb;
          ...
          color:#111;
        ">
          “Entregar la cuenta de cobro dentro de los cinco días hábiles siguientes al vencimiento de la misma, conforme a la forma de pago estipulada, con los respectivos soportes, resaltando que las mismas no deben ser acumuladas.”
        </div>
        <p style="margin-bottom:0;">
          Es decir que, si excedes el tiempo de entrega de la misma o acumulas dos cuentas de cobro,
          estarías incumpliendo esta obligación de tu contrato y la entidad puede tomar decisiones basadas en ello.
        </p>
      `
    }
  },
    {
    id: 'radicacion',
    title: '¿Por qué la cuenta no debe tener fecha de radicación mes vencido?',
    img: 'https://res.cloudinary.com/dqqeavica/image/upload/v1772497922/radicacion_yfxxw1.png',
    modal: {
      heroImg: '', // opcional: si luego quieres imagen aquí, coloca url
      html: `
        <p style="margin-top:0;">
          Todas las cuentas de cobro y facturas deberán radicarse dentro del mismo mes en que se ejecuten las actividades o se presten los servicios y, 
          en todo caso, antes del último día hábil de cada mes. Esto con el fin de garantizar la adecuada <b>causación contable de retenciones y estampillas</b>, 
          así como el cumplimiento oportuno de las obligaciones tributarias dentro de los plazos establecidos.
        </p>
        <div style="
          background:#f8fafc;
          border:2px solid #e5e7eb;
  ...
  {
    id: 'usar_correctamente',
    title: '¿Cómo usar correctamente la App en el proceso de cuentas y trámites?',
    img: 'https://res.cloudinary.com/dqqeavica/image/upload/v1772417403/opcion_tutoriales_lc7gyx.png',
    modal: {
      heroImg: 'https://res.cloudinary.com/dqqeavica/image/upload/v1772453472/tUTORIALES_lanqhy.png',
      html: `
        <p style="margin-top:0;">
          Una forma eficiente de realizar los procesos es conociéndolos correctamente. Por esta razón,
          puedes capacitarte a tu ritmo con cada uno de los videos tutoriales dispuestos para la adaptación de esta gran App.
        </p>
        <p style="font-weight:900; color:#06402B;">
          Ruta rápida:
        </p>
        <div style="
          ...
          margin: 8px 0 0;
        ">
          <span class="chip">MENÚ</span>
          <span class="chip">GUIA DE USO</span>
          <span class="chip">TUTORIALES</span>
        </div>
      `
    }
  }
...
// Estado del carrusel
let __homeBannerTimer = null;
let __homeBannerIndex = 0;
let __homeBannerRunning = false;

function homeBannerElements_(){
  return {
    wrap: document.getElementById('home-banner-wrap'),
    viewport: document.querySelector('#home-banner-wrap .home-banner-viewport'),
    track: document.getElementById('home-banner-track')
  };
}

function renderHomeBanners_(){
  const { track } = homeBannerElements_();
  if(!track) return;

  track.innerHTML = '';
  HOME_BANNERS.forEach((b)=>{
    const slide = document.createElement('div');
    slide.className = 'home-banner-slide';
    slide.dataset.bannerId = b.id;
  ...
  // Coloca el track en el índice actual
  homeBannerGoTo_(__homeBannerIndex, false);
}

function homeBannerGoTo_(idx, animate=true){
  const { track, viewport } = homeBannerElements_();
  if(!track || !viewport) return;

  const max = HOME_BANNERS.length;
  if(!max) return;

  __homeBannerIndex = ((idx % max) + max) % max;

  if(!animate){
    track.style.transition = 'none';
    track.style.transform = `translateX(-${__homeBannerIndex * 100}%)`;
    // forzar reflow y reactivar transición
    // eslint-disable-next-line no-unused-expressions
    track.offsetHeight;
    track.style.transition = 'transform 650ms cubic-bezier(.2,.85,.2,1)';
    return;
  }

  // pequeño “leave” para sensación de salida a la izquierda
  viewport.classList.add('leaving');
  setTimeout(()=> viewport.classList.remove('leaving'), 350);

  track.style.transform = `translateX(-${__homeBannerIndex * 100}%)`;
}

function startHomeBanners_(){
  if(__homeBannerRunning) return;
  const { wrap, track } = homeBannerElements_();
  if(!wrap || !track) return;

  __homeBannerRunning = true;

  // Render inicial (por si todavía no existe DOM)
  renderHomeBanners_();

  // Autoplay: cada 5 segundos
  __homeBannerTimer = setInterval(()=>{
    homeBannerGoTo_(__homeBannerIndex + 1, true);
  }, 5000);
}

function stopHomeBanners_(){
  __homeBannerRunning = false;
  if(__homeBannerTimer){
    clearInterval(__homeBannerTimer);
    __homeBannerTimer = null;
  }
}

/* ================== MODAL BANNER ================== */
function openBannerModal_(banner){
  const modal = document.getElementById('bannerModal');
  const title = document.getElementById('bannerModalTitle');
  const body  = document.getElementById('bannerModalBody');
  if(!modal || !title || !body) return;

  title.textContent = banner.title || 'Información';

  const hero = (banner.modal && banner.modal.heroImg) ? String(banner.modal.heroImg).trim() : '';
  const html = (banner.modal && banner.modal.html) ? String(banner.modal.html) : '';

  body.innerHTML = `
    ${hero ? `<img class="banner-modal-hero" src="${hero}" alt="">` : ''}
    ${html}
  `;

  modal.setAttribute('aria-hidden','false');

  // al abrir, detener autoplay (opcional; si quieres que siga, quita esto)
  stopHomeBanners_();
}

function closeBannerModal_(){
  const modal = document.getElementById('bannerModal');
  if(!modal) return;
  modal.setAttribute('aria-hidden','true');

  // si seguimos en inicio, reiniciamos
...
// Cerrar clic por fuera (en el backdrop)
document.getElementById('bannerModal')?.addEventListener('click', (e)=>{
  if(e.target && e.target.id === 'bannerModal'){
    try{ playSoundOnce(SOUNDS.back); }catch(_){}
    closeBannerModal_();
  }
});

/* ================== HOOK: iniciar/detener según vista ==================
   Importante: lo hacemos “wrappeando” showView sin romper lo demás.
*/
(function hookShowViewForHomeBanners_(){
  ...
  showView = function(id){
    ...
    // cerrar modal si cambian de vista
    const modal = document.getElementById('bannerModal');
...
document.getElementById('btn-login').addEventListener('click', async ()=>{
  ...
  try{
    // La contraseña viaja en el CUERPO del POST. Antes iba en la URL del GET,
    // así que quedaba escrita en el historial del navegador, en la pestaña
    // Network y en los registros de Google.
    const res = await apiPost('loginContratista', { documento, password });
    ...
    // Si el documento aparece 2 veces ACTIVO, el backend devuelve secretariasActivas[]
    let secretariaElegida = String(res.secretaria || '').trim();
    if(Array.isArray(res.secretariasActivas) && res.secretariasActivas.length >= 2){

      // IMPORTANTE: aquí sí debemos cerrar el overlay para permitir que el usuario vea el modal de selección
      hideConnectingLoginLoader_();

      const opts = res.secretariasActivas.map(s => String(s||'').trim()).filter(Boolean);

      const swalRes = await Swal.fire({
        icon:'info',
        title:'Elige la Secretaría de Ingreso',
        showCancelButton:true,
        showConfirmButton:false,
        ...
        html: `
          <div style="display:flex; flex-direction:column; gap:10px; margin-top:10px;">
            ${opts.map(sec => `
              <button type="button" class="swal2-confirm swal2-styled" data-sec="${sec}" style="display:block; width:100%; margin:0;">
                ${sec}
              </button>
            `).join('')}
          </div>
        ...
        didOpen: ()=>{
          ...
          if(!container) return;
          container.querySelectorAll('button[data-sec]').forEach(btn=>{
            btn.addEventListener('click', ()=>{
              const sec = btn.getAttribute('data-sec') || '';
              Swal.close({ isConfirmed:true, value: sec });
            });
          });
        }
      });
      ...
      secretariaElegida = String(swalRes.value || '').trim();
      if(!secretariaElegida){
        return;
      }

      // Reabrir overlay para continuar el flujo sin parpadeo
      showConnectingLoginLoader_();
    }

    // ACTIVO
    currentUser = { documento: res.documento, secretaria: secretariaElegida, supervisor: (res.supervisor || '') };

    // Determinar incompleto con base en la SECRETARÍA elegida (no con res.incompleto)
    let incompletoSel = false;
    try{
      /* El backend ya manda el INICIO dentro de la respuesta del login cuando no
         hay que elegir secretaría. Antes se hacían tres viajes seguidos
         (login -> getInicio -> bootstrapInicio, y el último repetía getInicio por
         dentro). Si por lo que sea no viene, se pide como siempre. */
      const inicioSel = (res.inicio && String(res.inicio.secretaria||'').trim() === String(currentUser.secretaria||'').trim())
        ? res.inicio
        : await apiGet('getInicio', { documento: currentUser.documento, secretaria: currentUser.secretaria });

      const faltantes = [];
      const campos = [
        'expedida','telefono','direccion','correo','municipio',
        'tipoCuenta','cuenta','banco','eps','pension','arl'
      ];

      for(const k of campos){
        const v = inicioSel ? inicioSel[k] : '';
      ...
      if(faltantes.length === 0){
        incompletoSel = false;
      }else if(faltantes.length === 1 && faltantes[0] === 'expedida'){
        incompletoSel = false;
      }else{
        incompletoSel = true;
      }

      // También actualiza supervisor desde la fila ACTIVO seleccionada
      if(inicioSel && inicioSel.supervisor){
        currentUser.supervisor = inicioSel.supervisor;
      }
    }catch(_){
      // Si falla, conserva el comportamiento anterior como fallback
      incompletoSel = !!res.incompleto;
    }
    ...
    if(incompletoSel){
      ...
      // Debemos cerrar overlay para mostrar política y diálogos normales
      hideConnectingLoginLoader_();
      ...
      await Swal.fire({
        ...
        html: 'Esta App será tu guía en el proceso contractual 2026<br>Valida muy bien todos los datos que ingreses.',
    ...
    // Completo: pintar INICIO. Si el login ya trajo los datos, no hay tercer viaje.
    let d, list;
    if(res.inicio && Array.isArray(res.noticias) &&
       String(res.inicio.secretaria||'').trim() === String(currentUser.secretaria||'').trim()){
      d = res.inicio;
      list = res.noticias;
    }else{
      const boot = await apiGet('bootstrapInicio', { documento: currentUser.documento, secretaria: currentUser.secretaria });
      d = boot?.inicio || {};
      list = boot?.noticias || [];
    }
    ...
    document.getElementById('inicio-cc').innerHTML =
      (d.documento || '') +
      (String(d.expedida || '').trim() ? (' <b>DE</b> ' + String(d.expedida || '').trim()) : '');
    ...
    // Cerrar overlay al final (justo antes de mostrar INICIO)
    hideConnectingLoginLoader_();
...
document.getElementById('btn-forgot-pwd')?.addEventListener('click', async ()=>{
  ...
  try{
    ...
    await Swal.fire({
      ...
      html:'Revisa tu WhatsApp registrado.<br>Puedes cambiarla cuando quieras desde <b>Actualizar contraseña</b>.',
      timer:6000, showConfirmButton:false
    });
  }catch(e){
...
// COMPARTIR PORTAL (con menú + sonidos)
document.getElementById('btn-compartir')?.addEventListener('click', ()=>{

  // Sonido al abrir el menú de compartir
  try{ playSoundOnce(SOUNDS.success); }catch(_){}

  Swal.fire({
    icon: 'success',
    title: 'COMPARTE LA APP PARA INSTALAR',
  ...
  setTimeout(()=>{
    ...
    if (btnWA){
      btnWA.addEventListener('click', async ()=>{
        ...
        const url = "https://tinyurl.com/GDF-CONTRATISTA-APP";
        const txt = "Instala la *App Contratista* 📲\n" + url;
    ...
    if (btnQR){
      btnQR.addEventListener('click', async ()=>{
        ...
        await Swal.fire({
          ...
          imageUrl: 'https://res.cloudinary.com/dqqeavica/image/upload/v1775413403/Contratista_2_xi2jzj.png',
          imageAlt: 'QR para instalar',
          confirmButtonText: 'Cerrar'
        });

        // Sonido "atrás" al volver a login
        try{ playSoundOnce(SOUNDS.back); }catch(_){}
        showView('view-login');
      });
    }
...
// Botón COMUNICADOS (sin recarga innecesaria)
document.getElementById('go-comunicados')?.addEventListener('click', ()=>{
  playSoundOnce(SOUNDS.login);
  overlay.classList.remove('open');
  showView('view-comunicados'); // usa NOTICIAS_CACHE ya cargado al iniciar sesión
});

document.getElementById('btn-logout').addEventListener('click', ()=>{

  // 1) Cierra cualquier SweetAlert que haya quedado abierto del usuario anterior
  try{ Swal.close(); }catch(_){}

  // 2) Limpia estados de "una vez por sesión"
  try{ sessionStorage.removeItem(NOTICIAS_SOUND_KEY); }catch(_){}
  try{ CONTRATISTA_EMAIL_CACHE = ''; }catch(_){}

  // 3) Detiene carruseles y limpia cache visual de noticias (para que no queden datos del anterior)
  ...
  // 4) Cierra overlays/modales propios
  ...
  // 5) Resetea loaders por si quedaron "enganchados"
  try{
    hideConnectingLoginLoader_();
    suppressLoader = false;
    loadingCount = 0;
  ...
  // 6) Resetea por completo TODOS los formularios y estado de ingresar/corregir
  try{ resetIngresarCuentaView(); }catch(_){}
  try{ resetVistas(); }catch(_){}

  // 7) Limpia usuario actual y vuelve al login
  playSoundOnce(SOUNDS.logout);
  currentUser = null;

  // Limpia caches de registro/personales (firma/cumple) para evitar arrastre visual
  try{ regFirmaDataUrl=''; revokeObjUrl(regPreviewUrl); regPreviewUrl=''; }catch(_){}
...
(function initOverlayCategories(){
  ...
  // Click en categoría (toggle: abrir/cerrar)
  catButtons.forEach(btn=>{
    btn.addEventListener('click', ()=>{
      const id = btn.dataset.cat;
      if(!id) return;

      // Sonido "back" al hacer clic en categoría
      try{
      ...
      if(isOpen){
        ...
        // Si está cerrada, la mostramos y ocultamos las demás
        showCategory(id);
      }
    });
  });

  // Al cargar: NO abrir ninguna categoría (todas cerradas)
  hideAllCategories();

  // Cuando se abre el overlay con el botón MENÚ, mantener todas cerradas
...
document.getElementById('go-personales').addEventListener('click', async ()=>{
  ...
  // Abrir SOPORTE desde GUÍA DE USO
...
async function cargarInicio(){
  ...
  const d = await apiGet('getInicio', { documento: currentUser.documento, secretaria: currentUser.secretaria });
  document.getElementById('inicio-nombre').textContent = d.nombre || '';
  document.getElementById('inicio-sub').textContent = 'CONTRATISTA - ' + (d.secretaria || '');
  document.getElementById('inicio-cc').innerHTML =
  (d.documento || '') +
  ...
  document.getElementById('inicio-arl').textContent = d.arl || '';
  document.getElementById('inicio-supervisor').textContent = d.supervisor || '';
  currentUser.supervisor = d.supervisor || '';
  const firmaEl = document.getElementById('inicio-firma');
  ...
  /* ================== CORREO CONTRATISTA (para alertas) ================== */
let CONTRATISTA_EMAIL_CACHE = '';

async function getContratistaEmail_(){
  if(CONTRATISTA_EMAIL_CACHE) return CONTRATISTA_EMAIL_CACHE;
  if(!currentUser || !currentUser.documento) return '';
  try{
    const email = await apiGet('getContratistaEmail', { documento: currentUser.documento });
    CONTRATISTA_EMAIL_CACHE = String(email || '').trim();
    return CONTRATISTA_EMAIL_CACHE;
  }catch(_){
    return '';
  }
}
...
/* Cargar lista desde backend (20 últimas) */
...
/* Badge en Inicio: campana + contador + “Ver Noticias” */
function unreadNoticiasCount(items){
  if(!Array.isArray(items) || !items.length) return 0;
  let n=0; for(const it of items){ if(!isRead(it.id)) n++; }
  return n;
}
// Clave de sesión para controlar el sonido (una vez por sesión)
...
function refreshNoticiasBadge(){
  ...
  // Sonar "notify" solo una vez por sesión mientras existan no leídas
  try{
...
/* Click en etiqueta “Ver Noticias” abre COMUNICADOS */
document.addEventListener('DOMContentLoaded', ()=>{
  const label = document.getElementById('notif-label');
...
function calStatusColorClass(estado, secretaria){
  ...
  if(v === 'SECRETARÍA DE GOBIERNO Y SERVICIOS ADMINISTRATIVOS') return 'cal-event-chip sec-gobierno';
  if(v === 'SECRETARÍA DE HACIENDA') return 'cal-event-chip sec-hacienda';
  if(v === 'SECRETARÍA DE EDUCACIÓN, DESARROLLO ECONÓMICO Y SOCIAL') return 'cal-event-chip sec-educacion';
  if(v === 'SECRETARÍA DE PLANEACIÓN E INFRAESTRUCTURA') return 'cal-event-chip sec-planeacion';
  if(v === 'SECRETARÍA DE ASUNTOS AGROPECUARIOS') return 'cal-event-chip sec-agro';
  if(v === 'SECRETARÍA DE SALUD') return 'cal-event-chip sec-salud';
...
function calRenderMonth(cont, filterVal){
  ...
  for(let i=0; i<cells; i++){
    ...
    events.sort((a,b)=> calMinutesFromTimeStr(a.horaInicio||'') - calMinutesFromTimeStr(b.horaInicio||''));
      ...
      // Mostrar el nombre del evento en lugar de la secretaría
...
function calRenderDay(cont, filterVal){
  ...
  if(!events.length){
    ...
    events.forEach(ev => {
      ...
      const title = document.createElement('span');
      title.className = 'cal-evt-title';
      // En vista día también usamos nombreEvento como título
      title.textContent = ev.nombreEvento || '';
      ...
      // Aquí puedes dejar un detalle adicional, por ejemplo la secretaría
  ...
  for(let i=0;i<7;i++){
    ...
    if(!evs.length){
      ...
      evs.forEach(ev => {
        ...
        const title = document.createElement('span');
        title.className = 'cal-evt-title';
        // En vista semana usamos nombreEvento como título
        title.textContent = ev.nombreEvento || '';
...
const lines = [
  ...
  '<p><b>Secretaría:</b> ' + (ev.secretaria || 'No Ingresado') + '</p>',
...
function spInitSimplePicker(dSelId,mSelId,aSelId){
  ...
  // Año fijo 2026 (una sola opción seleccionada)
...
// Publicación: picker con mínimo = hoy + 3 días
function spInitPubPicker() {
  const dSel = document.getElementById('spPubDia');
  const mSel = document.getElementById('spPubMes');

  const minDate  = new Date();
  minDate.setDate(minDate.getDate() + 3);
  const minDay   = minDate.getDate();
  const minMonth = minDate.getMonth() + 1; // 1-based

  const mesesNombres = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];

  // Año (fijo, ya lo carga spInitSimplePicker)
  const aSel = document.getElementById('spPubAnio');
  if (aSel && !aSel.childElementCount) {
    const opt = document.createElement('option');
    opt.value = '2026'; opt.textContent = '2026'; opt.selected = true;
    aSel.appendChild(opt);
  }

  function refreshDias(mesSeleccionado) {
    if (!dSel) return;
    const diaActual = parseInt(dSel.value) || 0;
    dSel.innerHTML = '';
    const desde = (mesSeleccionado === minMonth) ? minDay : 1;
    for (let d = desde; d <= 31; d++) {
      const opt = document.createElement('option');
      opt.value = spPad2(d);
      opt.textContent = opt.value;
      if (d === Math.max(diaActual, desde)) opt.selected = true;
      dSel.appendChild(opt);
    }
  }

  // Poblar meses desde el mes mínimo
  if (mSel) {
    mSel.innerHTML = '';
    ...
    mSel.value = spPad2(minMonth);
    refreshDias(minMonth);

    mSel.addEventListener('change', () => {
      refreshDias(parseInt(mSel.value));
    });
  }
}
spInitPubPicker();
...
// Abrir SOLICITUD PRENSA desde menú
document.getElementById('go-solicitud-prensa')?.addEventListener('click', async ()=>{
  ...
  try{
    ...
    // d.cumple ya se usa; aquí necesitamos estado ACTIVO desde hoja CONTRATISTAS.
    // Usamos getInicio para validar estado CC?
    const s = SS; // solo referencia lógica, en front no se usa (comentario explicativo)
  }catch(_){}
  try{
    // Reutilizamos getDatosPersonales + getEstadoCuenta no exponen CC, así que pedimos al backend una acción específica.
    const info = await apiGet('getSolicitudPrensaBase', { documento: currentUser.documento });
    if(!info || !info.found){
      await Swal.fire({
        icon:'info',
        title:'NO TIENES ACCESO A ESTA OPCIÓN',
        text:'Tu información de contrato aún no está completa.',
        timer:4000,
        showConfirmButton:false
      });
      return;
    ...
    if(String(info.estado||'').toUpperCase() !== 'ACTIVO'){
      await Swal.fire({
        ...
        text:'Solo los contratistas ACTIVO pueden usar esta opción.',
        timer:4000,
        showConfirmButton:false
      });
      return;
...
document.getElementById('sp-enviar')?.addEventListener('click', async ()=>{
  ...
  if(!publicacion){
    Swal.fire({icon:'warning', title:'Debes seleccionar la Fecha de Entrega o Publicación'});
    return;
  }
  if(!cargo){
    Swal.fire({icon:'warning', title:'Debes diligenciar tu Cargo en la Secretaría'});
    return;
  }
  ...
  // Resumen con "No Ingresado" para opcionales vacíos
  ...
  const resumenHtml = [
    ...
    '<b>Hora de Terminación:</b> ' + valOrNI(horaTermAmPm),
    ...
    '<b>Fecha de Entrega o Publicación:</b> ' + publicacion,
    '<b>Cargo:</b> ' + cargo
  ].join('<br>');
  ...
  try{
    ...
    if(tel){
      const msg =
        ...
        'Agradecemos la comprensión en nuestros tiempos de respuesta según demanda o solicitudes.\n\n' +
  ...
  // ✅ SEGUNDO MENSAJE FIJO A LAURA
const telFijo = normalizeNumber57('3134365563');
if(telFijo){
  const msgFijo =
    'Hola *Lau*\n' +
    'Tienes una nueva solicitud en la *App Comunicaciones*\n' +
    'Ingresa para Asignar.';
  sendBuilderbotMessage(telFijo, msgFijo);
}
...
document.getElementById('go-estado-cuenta')?.addEventListener('click', async ()=>{
  ...
  if(!currentUser){
    Swal.fire({ icon:'warning', title:'Sesión inválida', timer:3000, showConfirmButton:false });
    return;
  }
...
function mostrarEstadoCuenta(res){
  ...
  if(!res || res.found === false){
    if(res?.reason === 'first'){
      Swal.fire({ icon:'info', title:'APENAS ES TU PRIMERA CUENTA', text:'Aún no hay estados para esta', timer:3000, showConfirmButton:false });
      return;
    }
    Swal.fire({ icon:'info', title:'NO HAY REPORTES', text:'No haz Ingresado Cuentas aún', timer:3000, showConfirmButton:false });
    return;
  }
   ...
   case 'DEVUELTA': {
    ...
    case 'REVISADA POR SUPERVISOR':
      Swal.fire({
        ...
        html:`Tu <b>Cuenta ${informe}</b> por valor de <b>${valorCOP}</b> ya fue revisada por tu Supervisor(a)<br><br>A la espera de aprobación por la oficina de Contratación`,
        timer:8000,
        showConfirmButton:false
      });
      break;
    case 'APROBADA':
      Swal.fire({
        ...
        html:`Tu <b>Cuenta ${informe}</b> por valor de <b>${valorCOP}</b> ya fue APROBADA por la oficina de Contratación<br><br>Realiza Plan de Pagos en el SECOP II`,
        timer:8000,
        showConfirmButton:false
      });
      break;
    case 'PLAN DE PAGOS':
      Swal.fire({
        ...
        html:`Tu <b>Cuenta ${informe}</b> por valor de <b>${valorCOP}</b> está pendiente de Aceptación de PLAN DE PAGOS por tu Supervisor(a)`,
        timer:8000,
        showConfirmButton:false
      });
      break;
    case 'CERRADA':
      ...
      Swal.fire({
        ...
        html:`Tu <b>Cuenta ${informe}</b> por valor de <b>${valorCOP}</b> tiene ORDEN DE PAGO por la Oficina de Contabilidad<br><br>en breve se Emitirá a la oficina de Tesorería`,
        timer:8000,
        showConfirmButton:false
      });
      break;
    case 'ORDEN DE PAGO':
      Swal.fire({
        ...
        html:`Tu <b>Cuenta ${informe}</b> por valor de <b>${valorCOP}</b> Está a la espera de EGRESO por la oficina de Tesorería`,
        timer:8000,
        showConfirmButton:false
      });
      break;
    case 'EGRESO':
      ...
      Swal.fire({
        icon:'success',
        title:'PAGADA',
        html:`Tu <b>Cuenta ${informe}</b> por valor de <b>${valorCOP}</b> ya fue PAGADA por la oficina de Tesorería`,
        timer:7000,
        showConfirmButton:false
      });
      break;
    default:
      Swal.fire({
        icon:'info',
        title:'Estado: '+estado,
        html:`Tu <b>Cuenta ${informe}</b> por valor de <b>${valorCOP}</b> está en estado: ${estado}`,
        timer:5000,
        showConfirmButton:false
      });
      break;
  ...
  document.getElementById('go-dian')?.addEventListener('click', ()=>{
  playSoundOnce(SOUNDS.login);
  overlay.classList.remove('open');
  window.open(
...
document.getElementById('go-descargar-egresos')?.addEventListener('click', async () => {
  ...
  const telefono = (document.getElementById('inicio-contacto')?.textContent || '').trim();
  if (!telefono) {
    Swal.fire({ icon: 'warning', title: 'Sin teléfono registrado', text: 'Actualiza tu número en Datos Personales.' });
    return;
  }

  try {
    const data = await apiGet('getEgresosContratista', {
      documento: currentUser.documento,
      supervisor: currentUser.supervisor || ''
    });
    ...
    if (!data || !data.found) {
      await Swal.fire({
        ...
        text: 'No se encontraron egresos con estado PAGADA para tu contrato.',
        timer: 4000,
        showConfirmButton: false
      });
      return;
    ...
    const nombre   = data.nombre   || currentUser.documento;
    const contrato = data.contrato || '';

    const lineas = (data.egresos || []).map((eg, idx) => {
      const emoji = emojis[idx] || (idx + 1) + '.';
      let linea = emoji + '  *' + eg.cn + '*';
      if (eg.du) linea += ' y *' + eg.du + '*';
      linea += ' de *' + eg.fecha + '*';
      return linea;
    });
    ...
    const msg =
      'Estimado *' + nombre + '*\n' +
      'Tus *Egresos* del *contrato ' + contrato + '* son:\n' +
      lineas.join('\n') + '\n\n' +
      'Cordialmente,\n\n' +
      '*Gobierno Digital - Tesorería*\n' +
      '> Alcaldía de Flandes';

    sendBuilderbotMessage(telefono, msg);

    await Swal.fire({
      icon: 'success',
      title: '¡EGRESOS ENVIADOS!',
      text: 'El mensaje con tus egresos fue enviado a tu WhatsApp registrado.',
      timer: 3500,
      showConfirmButton: false
    });
...
document.getElementById('go-borrador')?.addEventListener('click', async ()=>{
  ...
  overlay.classList.remove('open');
  if(!currentUser){
    Swal.fire({icon:'warning', title:'Sesión inválida'}); return;
  }
  try{
    // RESET DURO: evita “cache visual” si cambia secretaría/supervisor
BA_STATE = { informe:1, obligaciones:[], actividades:[], cuentaExiste:false, activo:false, nombre:'', total:'' };
...
if(totInput) totInput.value = '';
    ...
    const data = await apiGet('getBorradorActividades', { 
  documento: currentUser.documento, 
  supervisor: currentUser.supervisor || '' 
});
  if (
  data.blocked &&
  !['INGRESADA','REVISADA POR SUPERVISOR'].includes(String(data.estado||'').toUpperCase())
){
  await Swal.fire({
    icon:'info',
    title:'AÚN NO PUEDES CREAR BORRADOR',
    text:'Después de Aprobada por Contratación tu cuenta anterior,´podrás crear borradores',
    timer:7000,
    showConfirmButton:false
  });
  showView('view-inicio');
...
if (String(data.estado||'').toUpperCase() === 'DEVUELTA'){
  await Swal.fire({
    icon:'info',
    title:'OPCIÓN NO VÁLIDA POR AHORA',
    text:'Toma la opción CORREGIR CUENTA',
    timer:5000,
    showConfirmButton:false
  });
  showView('view-inicio');
  return;
    ...
    if(data.activo === false){
      await Swal.fire({
        ...
        text:'Tu contrato no está ACTIVO actualmente.',
        timer:4000,
        showConfirmButton:false
      });
    }
...
function renderBorradorActividades(){
 const badge = document.getElementById('ba-informe');
  ...
  const wrap = document.getElementById('ba-list');
  wrap.innerHTML = '';
    // Limpia cualquier textarea anterior por seguridad (si el DOM llegara a persistir)
  for(let j=1;j<=26;j++){
    const old = document.getElementById('ba-act-'+j);
    if(old) old.value = '';
  }
  ...
  for(let i=0;i<n && i<26;i++){
    ...
    lab.textContent = 'Actividades por Obligación ' + idx + ':';
...
const BANKS = [
"BANCOLOMBIA","DAVIVIENDA","BANCO DE BOGOTÁ","BANCO CAJA SOCIAL","BANCO POPULAR","BANCO BBVA COLOMBIA S.A.","BANCO AV VILLAS","BANCO DE OCCIDENTE","BANCO AGRARIO DE COLOMBIA","BANCAMIA S.A.","BANCO SANTANDER COLOMBIA","BANCO FALABELLA","BANCO MUNDO MUJER S.A.","NEQUI","DAVIPLATA","DALE","NU","BANCOOMEVA S.A.","BANCO PICHINCHA S.A.","BANCO ITAU","CITIBANK COLOMBIA","BANCO COOPERATIVO COOPCENTRAL","DING","GLOBAL66","IRIS","JFK COOPERATIVA FINANCIERA","FINANCIERA JURISCOOP SA COMPAÑIA DE FINANCIAMIENTO","ALIANZA FIDUCIARIA","BAN100","BANCO SERFINANZA","BOLD CF","COINK SA","COTRAFA","CREZCAMOS","COLTEFINANCIERA","CONFIAR COOPERATIVA FINANCIERA","CFA COOPERATIVA FINANCIERA","BANCO UNION","SCOTIABANK COLPATRIA","BANCO GNB SUDAMERIS","BANCO J.P. MORGAN COLOMBIA S.A.","BANCO FINANDINA S.A. BIC"
];
...
function bindQuery(inputId, datalistId){
  ...
  let t = null;
  let abortCtrl = null;
  const cache = new Map(); // cache por término
  ...
  input.addEventListener('input', ()=>{
    ...
    t = setTimeout(async ()=>{
      ...
      try{
        ...
        const txt = await r.text();
        const j = (r.ok && !apiEsHtml_(txt)) ? JSON.parse(txt) : null;
...
let IC_EDIT_ROW = null;    // fila en CUENTAS cuando es corrección
...
function pad2(n){ return String(n).padStart(2,'0'); }
  ...
  const IS_MOBILE_INGRESAR = isMobileDevice(); // true en móviles, false en desktop
...
document.getElementById('go-corregir-cuenta')?.addEventListener('click', async ()=>{
  ...
  try{
    const base = await apiGet('getCorreccionCuentaData', {
  documento: currentUser.documento,
  supervisor: currentUser.supervisor || '',
  secretaria: currentUser.secretaria || ''
});
...
if(base.corregible === false){
  await Swal.fire({
    ...
    html:'tu anterior cuenta se encuentra en un Estado que no requiere Corrección',
    timer:6000,
    showConfirmButton:false
  });
  return;
    ...
    IC_STATE.contrato   = String(base.contrato||'').trim(); // puede venir vacío, no hace daño
    IC_STATE.nombre     = ''; // no se necesita para corrección en esta vista
    // El supervisor de la sesión (el de la secretaría elegida al entrar) NO se
    // puede perder: es lo único que distingue las cuentas de un contratista con
    // dos contratos. Antes iba vacío y el guardado terminaba en la otra cuenta.
    IC_STATE.supervisor = String((currentUser && currentUser.supervisor) || '').trim();
...
// evidData SOLO guardará dataURL nuevas que el usuario cargue en esta corrección.
// Se inicializa vacío para NO confundir URL vieja con “data nueva”.
...
IC_STATE.bankPlanUrls = {
  ...
  otroR:      String(base.otroRUrl || '')
      ...
      // Precargar fechas del periodo relacionado (I..N) desde backend
    ...
    const icFinEl = document.getElementById('ic-fin');
    const icDiaRat2El = document.getElementById('ic-diaRat2');
    const icMesRat2El = document.getElementById('ic-mesRat2');

    if(icInicioEl)  icInicioEl.value = String(base.inicioRatificar || '');
    if(icDiaRat1El) icDiaRat1El.value = String(base.diaRatificar1 || '');
    if(icMesRat1El) icMesRat1El.value = String(base.mesRatificar1 || '');

    if(icFinEl)  icFinEl.value = String(base.finRatificar || '');
    if(icDiaRat2El) icDiaRat2El.value = String(base.diaRatificar2 || '');
    if(icMesRat2El) icMesRat2El.value = String(base.mesRatificar2 || '');
    
    // Pintar vista con modo corrección
    renderIngresarCuenta();

    // Precargar relación de pago
    const saldoEl = document.getElementById('ic-saldo');
    if(saldoEl){
    ...
    const facturaNumEl = document.getElementById('ic-facturaNum');
    if(facturaNumEl){
      facturaNumEl.value = String(base.facturaNum || '');
    }
    ...
    if(fondoEl) fondoEl.value = formatCOPView(numPure(base.fondo));
    const riesgosEl = document.getElementById('ic-riesgos');
    if(riesgosEl) riesgosEl.value = String(base.riesgos||'');
    const caja1El = document.getElementById('caja1');
    if(caja1El) caja1El.value = String(base.caja1||'');
    
    const sena1El = document.getElementById('sena1');
    if(sena1El) sena1El.value = String(base.sena1||'');
    
    const icbf1El = document.getElementById('icbf1');
    if(icbf1El) icbf1El.value = String(base.icbf1||'');
    ...
    if(fon2El)  fon2El.value  = formatCOPView(numPure(base.fondo2));
    const rie2El  = document.getElementById('ic-riesgos2');
    const caja2El = document.getElementById('caja2');
    if(caja2El) caja2El.value = String(base.caja2||'');
    
    const sena2El = document.getElementById('sena2');
    if(sena2El) sena2El.value = String(base.sena2||'');
    
    const icbf2El = document.getElementById('icbf2');
    if(icbf2El) icbf2El.value = String(base.icbf2||'');
  ...
  // --- FORMATEAR A PESOS al cargar CORRECCIÓN (sin esperar a que el usuario toque el input) ---
try{
  // Formatear inputs COP que ya tienen valores precargados (incluye "0")
  (function formatCopOnLoadForCorreccion_(){
    if(IC_MODE !== 'correccion') return;

    const ids = [
      'ic-saldo','ic-cobro','ic-base','ic-salud','ic-fondo','ic-riesgos',
      'ic-base2','ic-salud2','ic-fondo2','ic-riesgos2'
    ];
    ...
    ids.forEach(id=>{
      ...
      const n = numPure(raw);
      el.value = (n === 0) ? '$ 0' : formatCOPView(n);
    });
  })();
}catch(_){}
...
showView('view-ingresar-cuenta');
  ...
  // Validación y bind de PDFs (NOVEDADES) - opcionales, 1MB, no cifrados
...
/* Botón menú: INGRESAR CUENTA (con sonido y navegación) */
document.getElementById('go-ingresar-cuenta')?.addEventListener('click', async ()=>{
 ...
 try{
    ...
    // Una sola llamada optimizada del backend (CONTRATISTAS + CUENTAS + validación)
    const base = await apiGet('getIngresarCuentaData', {
      documento: currentUser.documento,
      supervisor: currentUser.supervisor || '',
      secretaria: currentUser.secretaria || ''
    });

    /* La validación del contrato viene dentro de la misma respuesta: antes era
       un segundo viaje al servidor solo para preguntar si la columna Z estaba
       llena. Si el backend no la manda (despliegue viejo), se pregunta aparte. */
    const v = (base && base.puedeCrear) ? base.puedeCrear : await validarContratoParaIngresarCuenta_();
    if(!v || v.ok !== true){
      if(v && v.reason === 'z_empty'){
        await Swal.fire({
          icon:'info',
          title:'NO PUEDES CREAR CUENTA',
          html:'No completaste los datos del contrato previamente<br>Debes hacer clic en <b>DATOS DE PROCESO</b>, después en DATOS DE CONTRATO y finalmente en <b>Actualizar</b><br><br>Una vez termines este proceso, podrás ingresar tu primera cuenta.'
        });
      ...
      // Si por algún motivo no se encontró la fila ACTIVO con doc/sec/sup, o error lógico
      // no bloqueamos con el mensaje de Z vacío, pero evitamos romper el flujo.
    }
    ...
    if (bj === 'DEVUELTA') {
      await Swal.fire({
        icon:'info',
        title:'OPCIÓN NO VÁLIDA POR AHORA',
        html:'Toma la opción <b>CORREGIR CUENTA</b>',
        timer:5000,
        showConfirmButton:false
      });
      showView('view-inicio');
  ...
  IC_STATE.contrato      = String(base.contrato||'').trim();
    ...
    if(saldoEl){
      if(IC_STATE.saldoQ){
        // 2da cuenta en adelante: mostrar en formato pesos
        const inf = Number(IC_STATE.informe || 1);
        const saldoNum = numPure(IC_STATE.saldoQ);

        saldoEl.value = (inf >= 2) ? formatCOPView(saldoNum) : String(IC_STATE.saldoQ);
...
function driveUrlToThumb(url){
  ...
  const id = m ? m[1] : '';
  if(!id) return url; // si no extrae id, usa la url como está
...
function renderIngresarCuenta(){
  ...
  if(IC_MODE === 'correccion'){
    if(titleEl) titleEl.textContent = 'CORRECCIÓN DE CUENTA';
    if(descEl)  descEl.innerHTML = `
      <b style="color:#dc2626;">✍🏻 Edita sólo los campos que vas a actualizar para tu cuenta<br>📅 Vuelve a ingresar la Fecha de Radicación.<br>📤 Carga los archivos que te faltaron o reemplaza si adjuntaste erróneamente.</b><br><br>
      INFORME:
    ...
    if(titleEl) titleEl.textContent = 'CREACIÓN DE CUENTA';
    if(descEl)  descEl.innerHTML = `
      INFORME:
      <span id="ic-informe" class="chip">${String(IC_STATE.informe||1)}</span>
      &nbsp; DE:
  ...
  // Reasignar referencias de informe/total ahora que se re-renderizó HTML
...
bindPdfInputStrict('factElect',   { required:false, forbidEncrypted:true, maxMb: MAX_PDF_MB });
bindPdfInputStrict('otroPdf',     { required:false, forbidEncrypted:true, maxMb: MAX_ACT_ANEXOS_MB });

// Siempre iniciar oculto el bloque de "Más Documentos Anexos" al entrar a la vista
document.getElementById('ic-novedades-wrap')?.classList.add('hidden');

const FILE_LABELS = {
  bancaria:    'Certificación Bancaria.pdf',
  baucher1:    'Baucher Planilla.pdf',
  planaporte1: 'Planilla.pdf',
  baucher2:    'Baucher Planilla Anexa.pdf',
  planaporte2: 'Planilla Anexa.pdf',
  rutsg:       'RUT (Régimen Simple).pdf',
  factElect:   'Factura Electrónica.pdf',
  otroPdf:     'Otro Documento.pdf',
  noafp:       'Certificación NO Aportes Pensión.pdf',
  ...
  otrosi:      'OTROSI.pdf',
  cdpAc:       'CDP Adición.pdf',
  rpAc:        'RP Adición.pdf',
  actaL:       'Acta de Liquidación.pdf',
...
function resetFileUi_(id){
  ...
  if(inp){
    ...
    if(typeof inp.__refreshSavedHtml === 'function'){
      try{ inp.__refreshSavedHtml(); }catch(_){}
    }
  }
  if(st) st.innerHTML = '';
  if(clearBtn) clearBtn.style.display = 'none';
...
const bankIds = ['bancaria','baucher1','planaporte1','baucher2','planaporte2','rutsg','factElect','otroPdf','noafp','actAnexos'];
...
if(sel2){
  ...
  // Nuevo saldo dinámico + solidario/aporte
  const recompute = ()=>{
...
if(IC_MODE === 'correccion'){
  ...
  for(let i=0;i<n && i<26;i++){
    ...
    ta.id = 'ic-act-' + idx;
    ta.rows = 5;
    ta.placeholder = 'Describe la(s) actividad(es) que quieres ingresar, recuerda todo el esquema explicado al empezar esta sección.';
    ta.value = actVal;
    ...
    inp.addEventListener('change', async (e)=>{
      ...
      try{
        ...
        if(sizeMB > MAX_IMAGE_MB){
          await Swal.fire({
            icon:'warning',
            title:`Imagen ${idx} aún supera ${MAX_IMAGE_MB} MB`,
            text:'Usa una foto más liviana o baja la resolución antes de cargarla.'
          });
          inp.value = '';
          return;
        }
  ...
  ['ic-total','ic-inicio','ic-diaRat1','ic-mesRat1','ic-fin','ic-diaRat2','ic-mesRat2',
   ...
   'ic-solidario','ic-aporte','ic-solidario2','ic-aporte2', 'caja1','sena1','icbf1','caja2','sena2','icbf2'
  ].forEach(id=>{
    const el = document.getElementById(id);
    if(el){ el.value=''; }
  });
...
// Garantiza que el modal de Cumple tenga opciones (si aún no están cargadas)
function ensureCumpleModalOptions(){
  ...
  if (aSel && !aSel.childElementCount){
    // Rango amplio para periodos (ajústalo si deseas)
...
function esHabil(d){ // d = Date
  ...
  // Diciembre: todos los días habilitados excepto feriados (según tu regla especial)
  if (mm==='12'){
...
function abrirPickerRadicadoLimitado(){
  ...
  // Junta hoy + 2 próximos días hábiles (regla especial para diciembre/festivos 2026)
  while(count<3){
    if(esHabil(cursor)){
  ...
  // Configura Mes/Año según la primera opción
  const d0 = opciones[0];
...
document.getElementById('ic-guardar')?.addEventListener('click', async ()=>{
  ...
  try {
    ...
    try{
      ...
      if(r && r.success){
        mostrarExitoCuenta_(IC_MODE === 'correccion');
        resetIngresarCuentaView();
        showView('view-inicio');
      } else {
  ...
  const esCorreccion = (IC_MODE === 'correccion');

  /* Si el fallo NO es un error de negocio del backend, el trabajo pudo haberse
     completado igual: se le pregunta al servidor antes de asustar al usuario. */
  if(!(e && e.__apiNegocio)){
    const espera = Swal.fire({
      title:'VERIFICANDO TU GUARDADO...',
      html:'Se perdió la respuesta del servidor.<br>Estamos comprobando si tu cuenta alcanzó a quedar guardada.<br><br><b>No cierres la App.</b>',
      allowOutsideClick:false, allowEscapeKey:false,
      didOpen: ()=>{ Swal.showLoading(); }
    });

    const resultado = await confirmarGuardadoCuenta_(IC_STATE.informe);
    await Swal.close();

    if(resultado === 'completo'){
      mostrarExitoCuenta_(esCorreccion);
      resetIngresarCuentaView();
      showView('view-inicio');
      return;
    }

    if(resultado === 'a_medias'){
      await Swal.fire({
        icon:'warning',
        title:'TU CUENTA QUEDÓ GUARDADA A MEDIAS',
        html:'Los datos y los archivos <b>sí se guardaron</b>, pero los documentos no terminaron de generarse.<br><br>' +
             '<b>NO vuelvas a enviarla</b>: se duplicarían los archivos en tu carpeta de Drive.<br><br>' +
             'Informa a la Oficina de Contratación para que la completen.',
        confirmButtonText:'Entendido'
      });
      showView('view-inicio');
      return;
    ...
    if(resultado === 'no_guardo'){
      await Swal.fire({
        icon:'error',
        title:'NO SE GUARDÓ',
        html:'La conexión se cayó antes de que el servidor guardara nada.<br><br>' +
             'Tu información sigue en pantalla: puedes <b>volver a enviarla</b> sin riesgo de duplicar archivos.',
        confirmButtonText:'Entendido'
      });
      return;
    }

    // 'desconocido': ni siquiera se pudo consultar. Lo peligroso aquí es reenviar.
    await Swal.fire({
      icon:'warning',
      title:'NO PUDIMOS CONFIRMAR TU GUARDADO',
      html:'Se perdió la conexión con el servidor y tampoco pudimos comprobar cómo quedó.<br><br>' +
           '<b>NO vuelvas a enviarla todavía.</b> Entra a <b>MI CUENTA DRIVE</b> y revisa si tus documentos ya están.<br>' +
           'Si no están, vuelve a intentarlo más tarde.',
      confirmButtonText:'Entendido'
    });
    return;
  }
    ...
    try {
      ...
      if (sizeMB > MAX_IMAGE_MB) {
        await Swal.fire({
          ...
          text: `La firma supera ${MAX_IMAGE_MB} MB. Usa una foto más liviana.`
        });
        regFirmaInput.value = '';
        regFirmaPreview.src = '';
        regFirmaPreview.style.display = 'none';
...
document.getElementById('reg-guardar').addEventListener('click', async ()=>{
  ...
  const correoRegex = /^[^\s@]+@gmail\.com$/i;
...
if(!requiredOk){
  Swal.fire({icon:'warning', title:'Faltan campos Requeridos'});
  return;
}
  if(!telefonoRegex.test(body.telefono)){ Swal.fire({icon:'warning', title:'Whatsapp inválido'}); return; }
  if(!correoRegex.test(body.correo)){ Swal.fire({icon:'warning', title:'Debe ser un correo Gmail'}); return; }
  if(!(body.numeroCuenta.length>=7 && body.numeroCuenta.length<=16)){ Swal.fire({icon:'warning', title:'Número de cuenta inválido'}); return; }

  // Validación exacta de municipios (expedida/municipio) en GS también, pero aplicamos verificación previa
  let exactMup = true;
  let exactExp = true;

exactMup = await apiGet('validateMunicipioExact', { nombre: body.municipio });

if(body.expedida){
  exactExp = await apiGet('validateMunicipioExact', { nombre: body.expedida });
}

if(!exactMup || !exactExp){
  Swal.fire({icon:'warning', title:'Municipio inválido', text:'Debe coincidir exactamente con la lista.'});
  return;
}

  const resumenHtml = [
    body.expedida && `<b>Municipio de Expedición:</b> ${body.expedida}`,
    body.telefono && `<b>Whatsapp:</b> ${body.telefono}`,
    body.direccion && `<b>Dirección:</b> ${body.direccion}`,
    ...
    if(r && r.success){
      ...
      if(numero57){
        const msg =
          ...
          'Para mi es importante que selecciones tu *Correo Personal* al abrir y aceptar esta invitación: https://tinyurl.com/AF-CALENDARIO-INSTITUCIONAL\n' +
          'Con esto sabré que estarás en línea con cada evento insitucional para potenciar nuestro sentido de pertenencia. Quedo atenta.\n\n' +
          '*Debes GUARDAR este número a tus contactos para recibir los mensajes de estados de cuenta y responde a cada mensaje*\n\n' +
          '¡Un abrazo!\n\n' +
...
function convertirNumeroATexto(n){
  ...
  function cMiles(n){ if(n<1000) return cCent(n);
    ...
    if(millones===1) return ("UN MILLÓN "+(resto?cMiles(resto):"")).trim();
...
/* ✅ NUEVO: recalcula siempre el campo Tiempo de ejecución desde los valores EDITADOS (meses/días) */
...
/* ✅ NUEVO: cuando el usuario edita meses/días, actualiza "Tiempo de ejecución" inmediatamente */
...
function calcularPlazo(){
  ...
  // Contar ambos extremos (incluye día final)
...
if(!contratoYaDiligenciado){
  ...
  // Resumen: solo campos editados (no vacíos)
    const resumen = [];
  if(body.numP) resumen.push(`<b>N° de Proceso SECOP II:</b> CPS-${body.numP}-2026`);
  if(body.fechaInicioISO) resumen.push(`<b>Fecha de Inicio:</b> ${body.fechaInicioISO}`);
  if(body.fechaTerminoISO) resumen.push(`<b>Fecha de Terminación:</b> ${body.fechaTerminoISO}`);
  if((body.fechaInicioISO || body.fechaTerminoISO) && (body.meses || body.dias)){
    resumen.push(`<b>Meses/Días:</b> ${body.meses||'0'} / ${body.dias||'0'}`);
  }
...
document.getElementById('p-guardar').addEventListener('click', async ()=>{
  ...
  // Validación exacta (expedida/municipio) si vienen
  if(body.municipio){
    const ok = await apiGet('validateMunicipioExact', { nombre: body.municipio });
    if(!ok){
      Swal.fire({icon:'warning', title:'Municipio de Residencia inválido', text:'Debe coincidir exactamente con la lista.'});
      return;
    }
  }
  ...
  if(body.expedida){
    ...
    if(!ok){
      Swal.fire({icon:'warning', title:'Municipio de Expedición inválido', text:'Debe coincidir exactamente con la lista.'});
      return;
    }
  }
...
if (!hayCambiosPersonales) {
  ...
  if(body.telefono && !telefonoRegex.test(body.telefono)){ Swal.fire({icon:'warning', title:'Whatsapp inválido'}); return; }
  if(body.correo && !correoRegex.test(body.correo)){ Swal.fire({icon:'warning', title:'Debe ser un correo Gmail'}); return; }
  if(body.numeroCuenta && !(body.numeroCuenta.length>=7 && body.numeroCuenta.length<=16)){ Swal.fire({icon:'warning', title:'Número de cuenta inválido'}); return; }
...
async function cargarContratoLectura(){
  ...
  const lines = [
    ...
    `<b>N° de Proceso SECOP II:</b> ${d.numProceso||''}`,
    ...
    `<b>Costos o Deducciones:</b> ${d.costos||''}`
  ];

  (d.obligaciones||[]).forEach((o,i)=>{ lines.push(`<b>Obligación ${i+1}:</b> ${o}`); });
...
async function cargarPersonalesLectura(){
  if(!currentUser) return;
 const d = await apiGet('getDatosPersonales', { documento: currentUser.documento, secretaria: currentUser.secretaria });
...
document.getElementById('soporte-enviar')?.addEventListener('click', async ()=>{
  ...
  const soporte = String(document.getElementById('soporte-text')?.value || '').trim();
  if(!soporte){
    Swal.fire({ icon:'warning', title:'Debes escribir tu solicitud' });
    return;
  }
  ...
  try{
    ...
    if(!base || base.found !== true){
      Swal.fire({ icon:'error', title:'No se encontró tu información' });
      return;
    }

    if(String(base.estado||'').toUpperCase() !== 'ACTIVO'){
      Swal.fire({ icon:'info', title:'Contrato INACTIVO', text:'Solo contratistas ACTIVO pueden usar soporte.' });
      return;
    }

    const nombre    = String(base.nombre    || '');
    const secretaria = String(base.secretaria || '');
    const telefono  = String(base.telefono  || '').trim();
    ...
    const r = await apiPost('guardarSoporteContratista', payload);

    if(!r || r.success !== true){
      Swal.fire({ icon:'error', title:'No se pudo guardar', text:'Intenta nuevamente.' });
      return;
    }

    // Mensaje 1 → al contratista
    if(telefono){
      const msg1 =
        'Estimado(a) *' + nombre + '*\n' +
      ...
      sendBuilderbotMessage(telefono, msg1);
    }
...
document.getElementById('st-enviar')?.addEventListener('click', async ()=>{
  ...
  const resumenHtml = [
    ...
    '<b>Secretaría:</b> ' + (secretaria || 'No Ingresado'),
  ...
  try{
    ...
    if(tel){
      const msg =
        ...
        'Agradecemos tu comprensión en nuestros tiempos de respuesta según demanda.\n\n' +
        'Cordialmente;\n\n' +
        '*Tesorería*\n' +
        '> Alcaldía de Flandes';
    ...
    Swal.fire({
      ...
      text:'Será respondida, según los tiempos y demanda de solicitudes.',
      timer:5000,
      showConfirmButton:false
    });
...
// Inicializador selectores básicos (puedes reutilizar si ya está initPickerBase)
(function initPickerCumplePersonales(){
  ...
  if(aSel && !aSel.childElementCount){
    // Mismo rango que registro: 1936..2008 (ajústalo si deseas)
...
// En el guardado de personales, asegúrate de usar __cumpleISO_personales:
...
// Contrato: año fijo 2026; reutilizable para inicio/termino
...
function icEsHabil(d){
  ...
  // Nunca permite sábados ni domingos
  if (dow === 0 || dow === 6) return false;

  // Nunca permite días marcados como festivos
  if (IC_FERIADOS_2026.has(key)) return false;

  // Todo lo demás es hábil
  return true;
}
function icAbrirRadicado(){
  ...
  if(!sub && box){
    ...
    sub.id = 'icRadSub';
    sub.className = 'muted';
    sub.style.margin = '6px 0 10px';
    sub.style.fontWeight = '700';
    box.insertBefore(sub, box.querySelector('.picker-grid') || null);
  }
  ...
  function addHabilDaysFrom_(startDate, count){
    const out = [];
 ...
 // ===== Decidir conjunto de opciones =====
  ...
  opciones.forEach(dt=>{
    ...
    opt.textContent = icPad2(dt.getDate());
    dSel.appendChild(opt);
  });

  function syncMesAnioFromDia(){
    const dd = (dSel.value || '').trim();
    const info = byDay.get(dd);
    if(!info) return;

    __icRadYear  = info.year;
    __icRadMonth = info.month;

    mSel.innerHTML = '';
    aSel.innerHTML = '';

    const optM = document.createElement('option');
    optM.value = info.month;
    optM.textContent = info.monthName;
    mSel.appendChild(optM);

    const optY = document.createElement('option');
    optY.value = info.year;
    optY.textContent = info.year;
    aSel.appendChild(optY);
  }

  if(dSel.options.length) dSel.selectedIndex = 0;
  syncMesAnioFromDia();
  dSel.onchange = syncMesAnioFromDia;
...
async function icConfirmarRadicado(){
  ...
  // Validación: la fecha de radicación no puede estar dentro del periodo relacionado
  const inicioVal = document.getElementById('ic-inicio')?.value || '';
  const finVal    = document.getElementById('ic-fin')?.value    || '';
  ...
  // Impide Fecha de Radicación en medio de Periodo Relacionado
  if (dInicio && dFin && dFechaSel && dFechaSel >= dInicio && dFechaSel <= dFin){
    await Swal.fire({
      icon: 'info',
      title: 'NO PUEDES RADICAR EN ESTA FECHA',
      html: 'La Fecha de radicación seleccionada no puede estar dentro del periodo relacionado<br>Debes seleccionar al menos un (1) día hábil después de la Fecha de Fin de Periodo relacionado',
      confirmButtonText: 'OK'
    });
...
function esHorarioHabilAhora(){
  ...
  const dow = ahora.getDay(); // 0 Domingo, 6 Sábado
  if(dow === 0 || dow === 6) return false;
 ...
 document.getElementById('go-cuenta-drive')?.addEventListener('click', async ()=>{
  ...
  try{
    const res = await apiGet('getCuentaDrive', { documento: currentUser.documento, supervisor: currentUser.supervisor || '' });
    if(!res || !res.found){
      await Swal.fire({
        icon:'info',
        title:'NO HAZ INGRESADO CUENTAS',
        text:'Esta opción se toma después de Ingresar una Cuenta',
        timer:4000,
        showConfirmButton:false
      });
      return;
    ...
    if(estadosValidos.indexOf(estado) === -1){
      await Swal.fire({
        icon:'info',
        title:'OPCIÓN NO VALIDA POR AHORA',
        text:'Solo verás esta opción en estado: INGRESADA, INCOMPLETA y/o APROBADA',
        timer:5000,
        showConfirmButton:false
      });
      return;
  ...
  swalExtraSoundOnceAfterIcon_({
    ...
    html: `
        Unificalos guiándote con la explicación en <b>GUIA DE USO</b> - opción TUTORIALES<br>
        Ya solo falta el cargue en el SECOP II<br><br>
        📧 Solo con el correo <b>${escapeHtml_(await getContratistaEmail_() || 'No registrado')}</b> tienes acceso a la carpeta. 📧
      `,
    confirmButtonText:'OK'
  }, 'https://res.cloudinary.com/dqqeavica/video/upload/v1773171750/descargar_tasnro.mp3')
);
  ...
  await Swal.fire({
    icon:'info',
    title:'DEBES IR A CORREGIR CUENTA',
    text:'Adjunta solo los documentos requeridos',
    confirmButtonText:'OK',
    allowOutsideClick:true,
    allowEscapeKey:true
  });
  ...
  return;
    ...
    if(estado === 'REPORTADA'){
      const r = await Swal.fire({
        ...
        html:'Ahora debes esperar que tu Supervisor la Revise<br><br>Si la Reportaste sin revisar o completar los Documentos, evita este reproceso en el próximo reporte.',
        confirmButtonText:'OK'
      });
      if(r.isConfirmed) openFolder();
      return;
  ...
  swalExtraSoundOnceAfterIcon_({
    ...
    html: `
          Verifica que los Datos estén correctos y completos.<br><br>
          📧 Solo con el correo <b>${escapeHtml_(await getContratistaEmail_() || 'No registrado')}</b> tienes acceso a la carpeta. 📧<br><br>
          Para realizar correctamente este proceso, revisa en el botón <b>GUIA DE USO</b> - opción TUTORIALES
        `,
    confirmButtonText:'IR A MI CARPETA DRIVE'
  }, 'https://res.cloudinary.com/dqqeavica/video/upload/v1773171684/ver_wqglyu.mp3')
);
      if(r.isConfirmed) openFolder();
      return;
    }
 ...
 const estado = String(res.estadoBJ||'').toUpperCase();
    ...
    if(['INGRESADA','INCOMPLETA'].indexOf(estado) === -1){
      Swal.fire({
        ...
        text:'No puedes reportar la cuenta 2 veces o Tomar esta opción si tu Cuenta está en otro estado de revisión',
        timer:6000,
        showConfirmButton:false
      });
      return;
    ...
    const post = await apiPost('marcarCuentaReportada', {
      documento: currentUser.documento,
      supervisor: currentUser.supervisor || '',
      secretaria: currentUser.secretaria || '',
      tipo: 'REPORTE_CUENTA'
    ...
    await avisarSiFalloElGrupo_(post);
    showView('view-inicio');
  }catch(e){
    Swal.fire({icon:'error', title:'Error', text:String(e.message||e)});
  }
...
function mostrarModalPrevioReportarCuenta_(){
  Swal.fire({
    ...
    html:'Si tu cuenta <b>ya ha sido Devuelta por tu SUPERVISOR o CONTRATACIÓN</b>, con honestidad toma la opción <b>CORRECCIÓN</b> para dinamizar la nueva revisión<br><br><b>Si no habías reportado antes esta cuenta</b>, toma la opción <b>REPORTE INICIAL</b>',
    showConfirmButton:false,
    showCancelButton:false,
    allowOutsideClick:true,
    allowEscapeKey:true,
  ...
  try{
    ...
    if(!res || res.found === false){
      Swal.fire({
        ...
        text:'Esta opción se toma posterior a la Aprobación de una cuenta',
        timer:5000,
        showConfirmButton:false
      });
      return;
    ...
    const r = await Swal.fire(
      ...
      html:`Corrobora lo siguiente en el Secop II:<br><br>1. Subida de Cuenta Unificada a Plan de Pagos<br>2. Planilla Validada correctamente<br>3. Estado Enviado a la entidad<br><b>📌 AL HACER ESTE REPORTE TE QUITAS EL ACCESO A LA CARPETA DRIVE, POR ESO DEBES CONSERVAR EL PDF UNIFICADO QUE USASTE PARA CARGAR PLAN DE PAGOS EN EL SECOP II 📌</b><br><br>¡Evita los Reprocesos!`,
      showCancelButton:true,
      confirmButtonText:'REPORTAR',
      cancelButtonText:'CANCELAR'
    ...
    const post = await apiPost('guardarPlanPagosReporte', {
      documento: currentUser.documento,
      supervisor: currentUser.supervisor || '',
      pago: egreso
    });
...
async function ejecutarReportarCorreccionPlan_(){
  ...
  try{
    ...
    // El aviso al grupo lo manda el backend (ver reportarCorreccionPlan en Código.gs)
    const post = await apiPost('reportarCorreccionPlan', {
      documento: currentUser.documento,
      supervisor: currentUser.supervisor || ''
    });
