...
const API_TIMEOUT_MS   = 45000; // corta la espera si el servidor no responde
 ...
 */
async function apiFetchJson_(url, opts = {}, etiqueta = '', maxIntentos = 1){
  let ultimoError = null;

  for(let intento = 1; intento <= maxIntentos; intento++){
    const ctrl = new AbortController();
    const to = setTimeout(()=>{ try{ ctrl.abort(); }catch(_){} }, API_TIMEOUT_MS);
    ...
    try{
      ...
      if(e && e.__apiReintentable){ ultimoError = e; }
        ...
        console.warn('[API] timeout', etiqueta, url);
        ultimoError = apiErrorReintentable_('El servidor tardó demasiado en responder. Revisa tu conexión e intenta de nuevo.');
      }else{
...
async function apiGet(action, params = {}){
  ...
  try{
    ...
    return await apiFetchJson_(url.toString(), { method: 'GET' }, action, API_MAX_INTENTOS);
  } finally { stopLoading(); }
}
