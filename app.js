...
const API_TIMEOUT_MS        = 45000;   // GET normales
...
document.getElementById('btn-login').addEventListener('click', async ()=>{
  ...
  try{
    ...
    try{
      const inicioSel = await apiGet('getInicio', { documento: currentUser.documento, secretaria: currentUser.secretaria });
    ...
    // Completo: bootstrap y luego ir a INICIO
...
document.getElementById('go-ingresar-cuenta')?.addEventListener('click', async ()=>{
 ...
 try{
    const v = await validarContratoParaIngresarCuenta_();
    if(!v || v.ok !== true){
